...
@implementation searchInMap
{
    ...
    UIImageView*    _floatpingview;//固定的那个大头针
...
- (void)viewDidLoad {
    ...
    _viewmapq.showsUserLocation = NO;
    
    _floatpingview = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 48, 68) ];
    _floatpingview.image = [UIImage imageNamed:@"redpingimg.png"];
    
    [self.contentView addSubview:_viewmapq];
    
    [self.contentView addSubview:_floatpingview];
...
- (void)mapView:(QMapView *)mapView regionDidChangeAnimated:(BOOL)animated
{
    ...
    _floatpingview.center = mapView.center;
    
    [SVProgressHUD showWithStatus:@"加载中..." maskType:SVProgressHUDMaskTypeClear];
    float   llllllat =  mapView.centerCoordinate.latitude;
    float   llllllng =  mapView.centerCoordinate.longitude;
    
    [SAppInfo getPointAddress:mapView.centerCoordinate.longitude lat:mapView.centerCoordinate.latitude block:^(NSString *address, NSString *city, NSString *err) {
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    {
        [SVProgressHUD showErrorWithStatus:@"请先输入搜索关键字"];
        return NO;
    }
    
    [self searchKeywords_V2:textField.text];
